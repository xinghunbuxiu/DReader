...
public class a {
    static final boolean a = (!a.class.desiredAssertionStatus());
    ...
    public a(ReaderEnv readerEnv, UmengManager umengManager, ConnectivityReceiver connectivity) {
        this.readerEnv = readerEnv;
        this.umengManager = umengManager;
        this.connectivityReceiver = connectivity;
        this.connectivityReceiver.addConnectLister(new IConnectChanged() {
            @Override
        ...
        try {
            ...
            DkApp.get();
            MiStatInterface.initialize(context, miAppId, DkApp.get().getMiAppKey(), this.connectivityReceiver.getDistChannel());
    ...
    public void a(c cVar, long j) {
        try {
            if (cVar.ai() && !cVar.l()) {
                ...
                this.umengManager.onEvent(str, a(j, 1000, 2000, 3000, 4000, 5000, 6000));
            }
        } catch (Throwable th) {
        }
    }
    ...
    public void a(long j, long j2, long j3) {
        try {
            ...
            this.g.add(hVar);
            l();
            this.umengManager.onEvent("P_STORE_FIRSTLOAD_V1", a(j3, 500, 1000, 2000, 4000, 6000, 8000));
        } catch (Throwable th) {
        }
    }
    ...
    private void l() {
        TaskHandler.postTask(new Runnable() {
            @Override
            public void run() {
              if (connectivityReceiver.isNetConnected()){
                  new WebSession();
               }
            }
        });
        ...
        try {
            if (this.n <= 0) {
                ...
                if (this.i >= 3 && this.j.endsWith("_store")) {
                    String str;
                    if (this.connectivityReceiver.isFreshInstall()) {
